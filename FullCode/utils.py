...
class Data():
    ...
    def get_slice(self, iList):
        ...
        for i, H in enumerate(HT):
            rows_to_delete = np.all(H == 0, axis=1)
            H = H[~rows_to_delete]
            col_to_delete = np.all(H == 0, axis=0)
            HT[i] = H[:, ~col_to_delete]


        for H in HT:
            edges = []
            for i in range(H.shape[0]):
                edges.append(H[i].nonzero()[0])
            hyperedges.append(edges)
            _, eigvec = getEigvec(H)
            eigvecs.append(eigvec)




        return [alias_inputs, HT, items, targets, node_masks, hyperedges, eigvecs]